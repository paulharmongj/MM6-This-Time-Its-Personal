...
plot(Annual ~ Year, data = gtemp, type = "o",pch = 20, cex = 1.4,col = c("dodgerblue"), ylab = ylab, xaxt = "n")
...
summary(m1)
lines(gtemp$Year,fitted(m1),col="magenta",lwd=2)
```
...
df.sps <- data.frame(cbind(sps1,sps2))
ggplot(df.sps, aes(x = c(sps1,sps2)) + geom_point() + geom_line() + theme_classic()
